...
Canonical architecture: `spec/architecture.md` (= design-spec.md v1.1.5 as of 2026-05-04).

## v1.1.5 amendment (2026-05-04)

Additive deltas that expand Sprint 1 by ~1 day; full detail in `architecture.md` Appendix E and `implementation-plan.md` Sprint 1 (v1.1.2):

1. **`documents` registry table** — file-hash dedup, S3 blob pointers (`original_s3_uri` + `docling_json_s3_uri`), parser-metadata snapshot. Written at FastAPI ingress.
2. **`parsed_doc_chunks` populated from MVP** — text + hierarchy (`parent_chunk_id`, `hierarchy_level`, `read_order`) + char-offsets emitted at parse. Embeddings + `doc_summary_context` backfilled Sprint 3.
3. **+`temperature`** first-class column on `extractions`; **+`extraction_quality_json`** reserved-shape column (NULL on Phase 1; populated Phase 1.5+ when calibration unlocks per-field scoring).
4. **+`status`** on `corrections` (`open`/`applied`/`rejected`/`superseded`).
5. **Structured `FieldProvenance`** Pydantic shape inside `extractions.provenance_json.field_provenance` (§7.6) — joins by `chunk_id` to `parsed_doc_chunks`.
6. **Reflexion bullet** in §8.3 — bridges Phase 1.5 decomposition and Phase 3 distillation.

Migrations `002`–`005` in `src/cc_distribution_parser/db/snowflake_migrations/` apply alongside `001_initial.sql`.
...
- `architecture.md` — TDD-style technical design (= design-spec.md v1.1.5; the CANONICAL doc)
