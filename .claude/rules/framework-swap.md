...
LangGraph is pinned `~=1.1` (minor-compatible on the 1.x line); checkpointer pinned `langgraph-checkpoint-sqlite ~=3.0`. Market Scout's 2026-04-21 finding: LangGraph carries 2026 API-churn risk. Critic W3 (R4 in risk register) mandated isolation so a future swap is a one-directory rewrite.

**Pin history.** The original spec named `~=0.2.0` but that release line was never published to PyPI; first public release was `1.x`. Pyproject was updated to `~=1.1` during Sprint 0 (see `pyproject.toml` comment at lines 35-37). All sections below were rewritten 2026-05-24 (Sprint 7) to use `1.x` reference versions; the isolation discipline is unchanged.
...
## Acceptable upgrade — minor version (1.x → 1.y)
...
## Major upgrade decision (1.x → 2.x)

LangGraph minor versions have historically introduced breaking changes. Treat every major bump as a **decision**, not a maintenance task.
...
- **Pinning `langgraph` to a major-version range (`>=1.0,<3.0`).** Defeats the whole point — locks in churn risk.
